...
Canonical repository guidance for Codex, Claude Code, and other coding agents working in this repository.
...
# Local native plugin smoke tests
codex plugin marketplace add /path/to/robium
codex plugin add robium@robium
# Claude Code uses: /plugin marketplace add /path/to/robium, then /plugin install robium@robium
```
...
- `.codex-plugin/` — native Codex manifest. It explicitly bundles `skills/`; Codex auto-discovers the standard `hooks/hooks.json` path. Users must review and trust plugin hooks before they run.
- `.agents/plugins/marketplace.json` — Codex-native repo marketplace used by the CLI and desktop app. Only `plugin.json` belongs inside `.codex-plugin/`.
- `.claude-plugin/` — Claude Code manifest + marketplace (single `robium` entry, `source: "./"`). Skills, hooks, and the Claude-only agent are auto-discovered.
...
- `docs/superpowers/` — historical design specs and implementation plans. Current AGENTS.md, skill instructions, validators, and repository layout take precedence when the old monorepo-era paths disagree.
