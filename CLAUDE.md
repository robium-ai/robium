...
- **Site / live-demo infrastructure** → the robium-website repo (Astro site + demo orchestrator; has its own CLAUDE.md with the deploy pipeline and Cloud Run facts). Demo *backends* live in robium-internal-apps.

The sibling repos are `robium-ai/robium-internal-apps` (applications + REGISTRY.md; private), `robium-ai/robium-apps` (public showcase of promoted apps), and `robium-ai/robium-website` (site + orchestrator, own CLAUDE.md); the engine-era skill-update policy below still governs any `skills/**` edit regardless of which repo you launch from.
