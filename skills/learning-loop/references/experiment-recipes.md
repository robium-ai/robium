...
trigger phrasing). No evals.yaml tasks: means the tasks column reads an em-dash placeholder
...
- **tasks**: passed/total task-check fixtures, populated only with
  --with-tasks and only when the skill has them; an em-dash placeholder otherwise (no
