...
- **Delegation posture: embed.** RViz2's own domain (displays, config files,
  ...
  connected whole.
- **Diagnose in order: Fixed Frame, then QoS, then sim time.** <!-- id: diagnose-order-frame-qos-time --> These three
...
- **Config files are the unit of reuse, not manual per-session setup.** <!-- id: config-file-reuse -->
...
**1. Launch RViz2 standalone:** <!-- id: launch-rviz2-standalone -->
...
**3. Set the Fixed Frame** <!-- id: set-fixed-frame-quickstart --> (Global Options panel, top of the Displays
...
**4. Save the config** <!-- id: save-config-quickstart --> once the display set is right: `File > Save Config
...
**Launch with a saved config.** <!-- id: launch-with-saved-config --> Pass `-d <path>.rviz` on the command line or
...
1. **Fixed Frame wrong or not broadcasting.** <!-- id: fixed-frame-check --> Global Options → Fixed Frame
...
2. **QoS mismatch between the display and the publisher.** <!-- id: qos-mismatch-check --> Each display has
   ...
   themselves.
3. **`use_sim_time` mismatch or a stalled `/clock`.** <!-- id: sim-time-clock-check --> In simulation, if
...
**Nav2 debugging display set.** <!-- id: nav2-debugging-display-set --> For debugging a navigation stack, add (on
...
- **No headless or web mode.** <!-- id: no-headless-web-mode --> RViz2 needs a local X11/Wayland display;
...
- **macOS has no native ROS 2**, <!-- id: macos-no-native-ros2-rviz --> so RViz2 (which links against ROS 2) runs
...
- **A stale config referencing a display plugin that isn't installed** <!-- id: stale-config-missing-plugin -->
...
- Upstream: [RViz2 documentation](https://docs.ros.org/en/jazzy/p/rviz2/)
  ...
  plugin/display implementations; fetched directly on 2026-07-10, and CLI flag
