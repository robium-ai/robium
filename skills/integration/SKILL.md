...
|---|---|---|
| Same process, same language | Direct call / shared object, no IPC<!-- id: same-process-direct-call --> | Don't wrap this in ROS 2 messaging "for consistency": it adds serialization cost and a node boundary for no isolation benefit. |
| Same-host, both ROS 2 nodes | ROS 2 topics/services/actions (default RMW, default DDS discovery)<!-- id: same-host-ros2-native-transport --> | Native; multicast discovery works unmodified on the host network or a single non-networked process. This is the default; deviate only for a stated reason. |
| Cross-host or cross-container, both ROS 2 (one system) | Still ROS 2 topics/services/actions, but with discovery **explicitly** configured: host networking, a Fast DDS Discovery Server, static CycloneDDS peers, or rmw_zenoh with a router<!-- id: cross-container-ros2-explicit-discovery --> | Multicast usually doesn't cross container/host boundaries by default. Don't reach for a non-ROS transport just because discovery needs setup; fix discovery, keep the transport native. See `references/comms-selection.md`. |
| Cross-boundary to a non-ROS peer (another team's service, a cloud API, a system that doesn't speak ROS/DDS) | gRPC (typed, streaming-capable, low overhead) or REST (simplest, most interoperable) at the boundary<!-- id: non-ros-boundary-grpc-rest --> | This is the one case where a non-ROS transport is the *default*, not the exception; don't leak DDS/RMW specifics across an org or system boundary. |
| High-throughput same-host data (large images, point clouds, mostly within one host) | Shared memory: ROS 2 intra-process comms / loaned messages, or zenoh's SHM path if already on rmw_zenoh<!-- id: high-throughput-shared-memory --> | Adopt only when profiling shows serialization/copy overhead is the actual bottleneck, not a default starting point. |
