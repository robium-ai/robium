...
- **Both verticals get equal treatment.** The ROS/navigation vertical tests
  ...
  both verticals.
- **Assert against config constants, not re-typed literals.** A regression
...
- **A measurement harness needs a guard that it didn't silently measure
  nothing.** A benchmark CLI that catches and prints per-device exceptions can
...
- **Gate every paid remote run behind a free local dry-run.** Before submitting
...
- **A free-space occupancy-map assertion must clear "unknown" gray, not just
  half-max** (nav-trial, 2026-07-10). ROS trinary occupancy PGMs encode
