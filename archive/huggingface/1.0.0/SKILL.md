...
skill catalog (`huggingface/skills` on GitHub, 25 skills as of this session
...
- **Install the upstream catalog before any Hub operation, if not already
  ...
  `.claude-plugin/marketplace.json` this session — the marketplace manifest's
  ...
  field is the one that actually resolves, and this session's own
...
- **Pull in additional upstream skills on demand, not all at once.** Once
  ...
  confirmed via direct fetch of the upstream README this session. Install
...
(confirmed via direct fetch of that collection page this session — roughly
...
- Upstream: [huggingface/skills GitHub
  ...
  catalog, and marketplace manifest — fetched directly this session,
  ...
  directly this session), [Open X-Embodiment (LeRobot format)
  collection](https://huggingface.co/collections/lerobot/open-x-embodiment)
  (fetched directly this session), [Hugging Face Hub dataset docs](https://huggingface.co/docs/hub/en/datasets-adding)
  (upload/format conventions, fetched directly this session). Sibling
