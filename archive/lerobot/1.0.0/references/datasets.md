...
README, all fetched directly via raw GitHub URLs this session (`main`
...
v3.0 (included in `lerobot >= 0.4.0`; the installed `lerobot` this session
...
and `docs/source/il_robots.mdx` this session; robot/teleop type strings and
...
command. A hosted viewer also exists at
...
via its `meta/info.json` this session) are already on v3.0 — this is
