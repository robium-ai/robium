...
- **Validate the pipeline with a tiny self-trained checkpoint; treat
  pre-0.6 hub checkpoints as unloadable.** Since 0.6.0, `--policy.path`
  ...
  processor-era checkpoint. See `references/policies-and-training.md`.
...
- **GPU vs Apple Silicon (MPS) vs CPU training expectations differ by an
  ...
  out MPS for small-sim work. CPU-only is not a training
