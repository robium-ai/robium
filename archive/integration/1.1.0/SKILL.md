...
- **Prefer ROS 2 native comms inside a ROS system.** If two modules are both
  ...
  system "just in case."
- **One process per container, unless there's a stated reason.** Each
...
- **DDS discovery across containers must be configured explicitly, never
  assumed.** Docker's default bridge network does not forward the multicast
  ...
  to defer.
- **Never write RMW/DDS/zenoh version or default-status claims from memory.**
...
**4. Write one Dockerfile per module.** Multi-stage: a build stage with the
...
**5. Wire modules with docker-compose.** One service per container, explicit
...
**6. Verify discovery, not just that containers start.** `docker compose up`
...
- **Rate mismatch** → split. A 1 kHz control loop and a 10 Hz planner have
...
- **Failure domain** → split. If module A crashing should not take down
...
- **Neither applies** → keep together. Two tightly coupled pieces with the
...
- **Docker's default bridge network drops DDS multicast.** The default
...
- **`network_mode: host` is Linux-only in the way this skill means it.**
...
- **rmw_zenoh needs its own router process.** Switching `RMW_IMPLEMENTATION`
...
- **`ROS_DOMAIN_ID` collisions are silent.** Two unrelated ROS 2 systems on
