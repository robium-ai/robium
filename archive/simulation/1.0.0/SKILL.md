...
- [ ] **Rate** — matches the real sensor's datasheet rate, not the tutorial's
  default (a lidar or camera simulated too fast/slow skews anything tuned
  against real hardware, e.g. a costmap update rate).
- [ ] **Noise model** — present and roughly matched to the real sensor's noise
...
- [ ] **Frame names** — `frame_id`/`child_frame_id` match the real robot's TF
  tree exactly, not a simulator-default name, so downstream consumers (Nav2,
  a perception node) don't need sim-only remapping.
- [ ] **Timestamps** — derived from the simulation clock, not wall-clock time.
- [ ] **`use_sim_time`** — set to `true` consistently across every node in the
...
- **Isaac Sim has no macOS support at all** — not the GUI, container, or pip
  ...
  workaround.
- **Gazebo's ROS 2 integration still needs Docker on macOS.** `gz sim` itself
