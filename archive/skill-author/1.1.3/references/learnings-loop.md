...
- **User-corrected approach** — the user overrode or corrected the
  agent's skill-guided approach mid-session ("no, use X instead", "don't
  do it that way here"). Record the exact correction and what the skill
  had suggested; a correction is the strongest single-observation signal
  that a skill's guidance and reality disagree.
...
### The evidence bar (write entries that can be absorbed)

Absorption (skill-updater Gate 1, hardening step 4) holds new knowledge to
a three-part promotion bar, so capture the three parts at logging time,
while they're cheap:

1. **The passing check** — what verified the fix/method (the test that
   passed, the command that exited clean, the run that went green).
   "Seemed to work" entries stay tentative.
2. **The named failure pattern** — the exact error message or symptom,
   verbatim. Exact strings become grep-able skill gotchas; paraphrases
   don't.
3. **The ruled-out dead-ends** — approaches tried and eliminated, with
   why. The dead-end often saves the next session more time than the fix.

An entry missing a part is still worth logging (capture is lossless —
unabsorbed notes cost nothing) — just expect it to wait in `learnings/`
until the missing evidence shows up. Two more capture habits that pay off
at hardening time: append a `(seen 2x)` count when the same friction
re-hits (recurrence is the strongest promotion signal, and the count makes
it visible), and route *project-local facts* (this app's port, this repo's
path) to the app's README/brief instead of `learnings/` — they're real
knowledge but never skill content.
...
6. **Prune** — hardening removes as deliberately as it adds. Act on every
   ...
   ever adds content is bloating the catalog, not hardening it. The
   `skill-refiner` skill is the systematic version of this step — its
   metrics script (`skills/skill-refiner/scripts/skill_metrics.py`)
   measures bloat and lists stale dated facts, and its four passes (bloat,
   duplication, staleness, usage review) extend the prune from
   within-one-skill to across the catalog. Run at least its scoped passes
   during every hardening session; run a full refine periodically.
