...
- **Recurrence rule:** anything appearing twice across learnings notes
  becomes a skill edit immediately — do not wait for a third occurrence.
- Every absorbed learning adds **one dated line to the `## Changelog`**
...
- **Always run `scripts/validate_skills.py` before committing any skill
  change** — fresh, mined, or hardened. A skill that fails the validator is
  not done.
- For eval design and description-wording tuning, **wrap Claude's own
  `skill-creator` skill** rather than reinventing evals or trigger-testing
...
1. `cp -r skills/_TEMPLATE skills/<new-skill-name>`, rename the dir to
   match the intended `name:`, and rename `SKILL.template.md` to `SKILL.md`
...
1. Read the target repo (an official demo repo, or an app built without
   robium) end to end — configs, launch files, Dockerfiles, glue code.
...
5. Mark each absorbed learning by appending `<!-- absorbed: YYYY-MM-DD -->`
...
- **New skill vs. deepen an existing one vs. add a reference file** —
  ...
  1. Does an existing skill already own this domain (same tool, same
     decision point)? If yes, the content is an edit to that skill, not a
     new one.
  ...
  3. Only create a brand-new skill when the content is a genuinely distinct
     decision point or tool with its own trigger surface — e.g. `nav2` vs
  ...
  4. If the new content would push a skill's `SKILL.md` body toward the
     ...
     the validator fails.
- **Body snippet vs. `examples/` file** — a snippet belongs directly in
...
- `references/mining-guide.md` — how to recognize a reusable pattern in an
  existing repo, trim it into a skill asset, and place it correctly.
...
- Upstream: Claude's built-in `skill-creator` skill (evals, description
  ...
  section 5 (the design source for this skill).
