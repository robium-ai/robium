...
- **Scale-to-zero, and state per-session cost.** `--min-instances=0` unless a
...
- **A boot that needs CPU must hold a request while it boots.** Cloud Run's
...
- **No UDP multicast anywhere on Cloud Run.** gz-transport and DDS discovery
...
- **Never write gcloud flag names or Cloud Run limits from memory.** CLI shape
...
**1. Build the image and push to Artifact Registry.** Two paths (both are real
...
**2. Deploy the container** with the robotics/sim flag set (values verified for a
...
**3. Map a same-site subdomain** (e.g. `demo.yourdomain.org` → the service) if
...
**Choose the billing mode — the central cost decision.**
...
- `--concurrency=1` gives per-connection instances with zero code, but *only*
...
- `--concurrency=4 --session-affinity` lets a page's status polls and stop calls
...
**Long-lived WebSockets (Foxglove bridge, PTY-over-ws).** Cloud Run supports
...
**Deploy auth from CI / servers (`GCP_SA_KEY`).** A service-account key (JSON,
...
**Fleet visibility (the "N of budget" count).** Query Cloud Monitoring
...
- **No UDP multicast → gz-transport needs a unicast relay.** Set
...
- **DDS multicast is equally absent.** `FASTDDS_BUILTIN_TRANSPORTS=UDPv4`
...
- **A health probe that connects-and-drops throttles the container mid-boot.**
...
- **Cold start includes image pull on a fresh node.** A ~2.5 GB ROS image adds
  30–90 s to the first boot on a node; say so on any user-facing page rather
  than letting it read as a hang.
- **Direct VPC egress: size the subnet for the whole instance pool.** If you
...
- **Public interactive shells need a real threat model, not just
  `--allow-unauthenticated`.** The exposures that matter on a browser-reachable
