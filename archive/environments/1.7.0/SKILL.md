...
- **Environment before code.** <!-- id: env-before-code --> Decide and record the environment strategy
  before writing application code. An undecided environment is an open risk,
  not a detail to fix later.
- **Preflight the machine before deciding.** <!-- id: preflight-with-doctor --> Run `npx robium-ai doctor --json`
...
- **Every project states its env strategy in the architecture brief.** <!-- id: record-strategy-in-brief --> If
  ...
  reads.
- **Local == remote is the acceptance test.** <!-- id: local-remote-parity-acceptance-test --> An environment strategy isn't
...
- **Never write image tags or version numbers from memory.** <!-- id: never-write-versions-from-memory --> Verify current
...
- **No — pure-Python (ML training/inference, data tooling, a plain script):** <!-- id: pure-python-uv-choice -->
...
- **Yes — ROS 2, system packages, or a robot's exact host OS matters:** <!-- id: ros2-system-deps-docker-choice -->
...
- **Both — ROS 2 in one place, a heavy pure-Python ML stack in another:** <!-- id: mixed-ros2-python-docker-uv-inside -->
...
- [ ] <!-- id: pinned-base-image-not-latest --> Base image is pinned to a specific tag (and ideally digest), not
  `latest` — so "remote" can't silently drift from "local".
- [ ] <!-- id: lockfile-single-source-of-truth --> `uv.lock` (or the container image itself) is the single source of
...
- [ ] <!-- id: clean-room-cache-caveat --> "Clean-room" claims name what was actually cold: `docker compose down
...
- **macOS has no native ROS 2.** <!-- id: macos-no-native-ros2-docker-required --> There is no supported native ROS 2 install
...
- **GPU containers need `nvidia-container-toolkit`, and it's Linux-only.** <!-- id: gpu-toolkit-linux-only -->
...
- **arm64 slim images need a compiler for source-only wheels.** <!-- id: arm64-slim-needs-build-essential --> On
  linux/arm64 a `python:3.x-slim` base fails at install time for any dependency
  ...
  non-slim base). (observed 2026-07-15, manip-trial)
- **Docker on macOS cannot see MPS — for ML/VLA on Apple Silicon this is a
  latency decision, not a preference.** <!-- id: docker-macos-no-mps-cpu-fallback --> Docker containers on macOS run in a
...
- **X11/Wayland forwarding vs headless + web viz.** <!-- id: x11-forwarding-breaks-remote-headless-default --> Forwarding a display out
...
- **A Mac wired to a no-internet robot LAN loses internet — reorder network
  services.** <!-- id: mac-network-service-order-robot-lan --> macOS ranks a wired (USB-Ethernet) service above Wi-Fi, so when
...
- **Real-robot bring-up networking is its own layer — before any container
  or lockfile matters.** <!-- id: real-robot-networking-own-layer --> IPv6 link-local first contact, netplan Wi-Fi
  reconfig, why the ROS↔browser bridge must run on the robot (DDS multicast
  crosses neither NAT nor Docker Desktop's VM), two-DHCP-servers-per-SSID,
  Wi-Fi MAC randomization, and validating a camera panel on a Mac with no
  V4L2 — all in `references/robot-networking.md` (tb4-teleop, 2026-07-25).
- **Some GPU apps have no local mirror on a Mac at all — the one sanctioned
...
- 1.7.0 (2026-08-02): add Platform gotchas [reasons: obs-environments-001] (applied by apply_deltas)
