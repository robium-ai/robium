...
- **Adding system packages to a uv-only project:** that's the signal to
  ...
  where Docker would forfeit the MPS accelerator (GPU containers are
  Linux-only, per Platform gotchas) — document the one `brew install` step
  in the project README and stay on uv.
