...
- **arm64 slim images need a compiler for source-only wheels.** On
...
- **Real-robot bring-up networking is its own layer — before any container
  or lockfile matters.** IPv6 link-local first contact, netplan Wi-Fi
...
- **Some GPU apps have no local mirror on a Mac at all — the one sanctioned
  exception to local==remote.** Isaac Sim / Isaac Lab (Omniverse Kit needs
