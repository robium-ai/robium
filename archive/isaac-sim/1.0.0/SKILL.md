...
- **Delegation posture: embed + links.** No upstream skill or plugin wraps
  ...
  References.
- **Prefer the official container for reproducibility.** `nvcr.io/nvidia/
...
- **Headless + livestream for remote work — not X11 forwarding.** A remote
  GPU server has no display; run Isaac Sim with `runheadless.sh` and stream
...
**Run the container.** `docker run --gpus all --network=host -e
