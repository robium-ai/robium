...
verified this session by direct `curl` of the raw source files cited in each
...
README, fetched directly this session — see the tag's exact semantics
...
fetched directly this session — flags read from its `gflags::DEFINE_*`
...
this session — this is the authoritative flag list, not a tutorial's subset
of it.
