...
- **Never recommend Gazebo Classic (`gazebo11`).** It reached end-of-life;
...
- **Bridge every topic explicitly, via a config file, not ad-hoc CLI
  bridges.** A one-off `ros2 run ros_gz_bridge parameter_bridge
...
- **GPU vs. software rendering.** Camera and `gpu_lidar` sensors go through
...
- **Upstream robot-demo launch files often can't run headless as-is.**
  ...
  parameterized.
- **On a ROS 2 system the `gz` CLI is vendored and needs a sourced env.**
...
- **Running `gz` in Docker.** A ROS 2 + Gazebo + `ros_gz` stack in a
  ...
  image from scratch.
- **macOS status.** `gz sim` itself ships native Jetty binaries for macOS
