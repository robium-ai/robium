...
- Cross-references: go to the sibling skill instead when the question is:
  - The Cloud Run deploy mechanics themselves, meaning the build → Artifact Registry
...
- **A demo is a product surface: the smoke test extends to it.** <!-- id: demo-smoke-extends-product-surface --> The app's
...
- **Scale-to-zero is non-negotiable; per-session cost is explicit.** <!-- id: scale-to-zero-non-negotiable-explicit-cost -->
...
- **One visitor, one instance, enforced in the gateway.** <!-- id: one-visitor-one-instance-gateway-enforced --> The session
  ...
  for isolation.
- **Lifecycle belongs to something that outlives the instance.** <!-- id: lifecycle-needs-host-level-orchestrator --> The
...
- **Be honest on the page.** <!-- id: honest-cold-boot-copy-on-page --> Cold-boot time (30–90 s, with self-restart
...
1. **App side** <!-- id: app-side-demo-scenario-gateway -->: add a `demo` scenario to the app: one launch = sim +
...
2. **Deploy** <!-- id: gcloud-run-deploy-demo-flags -->: Cloud Build the image, then (values verified for a
   Gazebo+Nav2 stack):
   ```bash
...
4. **Gate it** <!-- id: gate-with-demo-smoke-before-linking -->: demo smoke locally, then the same probes against the
...
| --- | --- | --- |
| **Mission-control page** (proven) | Start/Stop instance buttons, live boot terminal, fleet count, viewer opens on ready | Default. Honest about boot time; visitors see the machinery (which *is* the pitch for infra products).<!-- id: flow-mission-control-default --> |
| Deep-link only | One "open in Foxglove" link; connection cold-boots the instance | Minimal page work; boot happens behind the viewer's "connecting" spinner; needs the connection itself to hold CPU (request-based billing).<!-- id: flow-deep-link-minimal --> |
| Embedded viewer (self-hosted) | Viewer iframe in the page, no login | Best UX on paper; costs a self-hosted viewer build and iframe/browser-storage complexity. Deferred by robium.ai after trying it; revisit deliberately.<!-- id: flow-embedded-viewer-deferred --> |

**Choose the visualizer** (verified facts, 2026-07-13):

| Option | Login | Layout preload | Embeddable | Verdict |
| --- | --- | --- | --- | --- |
| app.foxglove.dev deep link (`/~/view?ds=foxglove-websocket&ds.url=<wss>`) | Required | ✗ (visitor imports the layout file once; link it on the page) | ✗ (`x-frame-options: DENY`) | Default: zero hosting, robotics users have accounts<!-- id: visualizer-foxglove-deeplink-default --> |
| Self-hosted Lichtblick (open-source Foxglove fork, MPL) | None | ✓ (`globalThis.LICHTBLICK_SUITE_DEFAULT_LAYOUT` build hook) | ✓ (you control headers) | Frictionless but you own the build + browser-storage kiosk-wipe; new-tab use is solid, iframe needs care<!-- id: visualizer-lichtblick-self-hosted --> |
| Foxglove embed SDK (embed.foxglove.dev) | Org members only | ✓ | ✓ | Paid tier + viewers must belong to your Foxglove org: internal portals, not public demos<!-- id: visualizer-foxglove-embed-sdk-internal --> |

**Instance lifecycle contract** <!-- id: instance-lifecycle-gateway-contract --> (the gateway, `examples/demo_gateway.py`):
...
**Show the demo where the proof lives.** <!-- id: demo-link-from-proof-card --> The homepage apps/proof card for
...
**Fleet/budget display.** <!-- id: fleet-budget-never-fetch-on-page-load --> Budget = `--max-instances`, stated statically
...
- **Gazebo discovery needs help on Cloud Run** <!-- id: gazebo-discovery-cloud-run-boot-watchdog --> (no multicast):
...
- **`Connection: close` on every hand-rolled HTTP response** <!-- id: connection-close-header-required -->: Cloud
...
- **SIGINT, not SIGTERM, stops `ros2 launch` as PID 1** <!-- id: sigint-not-sigterm-for-ros2-launch-pid1 -->: the kernel
  drops unhandled signals to PID 1 and launch installs no SIGTERM
  handler.
- **Session affinity cookies are SameSite-Lax** <!-- id: session-affinity-cookies-samesite-lax -->: they never flow to a
...
- **Request-based billing throttles CPU between requests** <!-- id: request-based-billing-cpu-throttle-needs-no-throttling -->: a sim boot
...
- **FastDDS shared memory misbehaves in Cloud Run** <!-- id: fastdds-shm-misbehaves-cloud-run -->:
  `FASTDDS_BUILTIN_TRANSPORTS=UDPv4` silences `open_and_lock_file` errors.
- **Concurrent instances must not share a `ROS_DOMAIN_ID`** <!-- id: concurrent-instances-domain-id-collision -->: the project's
...
- **A single local instance has nowhere to reroute a 503; needs takeable
  claims + abortable runs** <!-- id: single-instance-503-needs-abortable-takeable-claims --> (2026-07-15, user-corrected). On Cloud Run a
