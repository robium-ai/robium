...
- **TurtleBot 4: the Create 3 base is invisible to the Pi until you drop the
  stock `CYCLONEDDS_URI` interface restriction.** `/etc/turtlebot4/setup.bash`
  ...
  uncommenting `usb0` in the xml, a Create 3 reboot (`POST
  http://192.168.186.2/api/reboot`), ntpd resync, a full Pi reboot, and
...
- **Inspecting Create 3 (best-effort) topics from the CLI.** `ros2 topic echo`
