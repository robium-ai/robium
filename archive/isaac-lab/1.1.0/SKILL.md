...
- **On a cloud GPU, prefer the prebuilt Isaac Lab image over a source
  install.** `nvcr.io/nvidia/isaac-lab` (latest tag observed
...
**1. Confirm the GPU floor** — see the `isaac-sim` skill; do not proceed
until the target machine meets it.

**2a. Cloud GPU (preferred): pull the prebuilt Isaac Lab image.**
...
Isaac Lab from source on top of it** (per-release version pins matter —
...
**Verified Go2 locomotion walkthrough.** A battle-tested RSL-RL run on the
