#!/usr/bin/env python3
"""Catalog metrics dashboard for skill-refiner's bloat audit and staleness sweep.

Run from the robium repo root:

    python3 skills/skill-refiner/scripts/skill_metrics.py [--stale-days 90] [--skills-dir skills]
    python3 skills/skill-refiner/scripts/skill_metrics.py --history   # growth trend from archive/
    python3 skills/skill-refiner/scripts/skill_metrics.py --dupes     # cross-skill duplicate lines
...
def audit_skill(skill_dir: Path, stale_before: dt.date) -> dict:
    ...
    for f in [skill_dir / "SKILL.md", *sorted((skill_dir / "references").glob("*.md") if (skill_dir / "references").is_dir() else [])]:
