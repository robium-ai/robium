...
test('parseGeminiSkills reads enabled state and location', () => {
  const parsed = parseGeminiSkills(`Discovered Agent Skills:\n\nnavigation [Enabled]\n  Description: nav\n  Location:    /tmp/navigation\n\nros2 [Disabled]\n  Description: ros\n  Location:    /tmp/ros2\n`);
  assert.deepEqual(parsed, [
    { name: 'navigation', enabled: true, location: '/tmp/navigation' },
...
test('Gemini inspection falls back to concise extension output', async () => {
  ...
  const result = await inspectGeminiIntegration({
    exec, home: '/not-used', expectedPluginVersion: '0.4.0',
  });
...
test('Gemini managed copy is active when discovered and reports a stale install marker', async () => {
  const base = await mkdtemp(path.join(os.tmpdir(), 'robium-gemini-status-'));
  const home = path.join(base, 'home');
  const skill = path.join(home, '.gemini', 'skills', 'navigation');
  await mkdir(skill, { recursive: true });
  await writeFile(path.join(skill, 'SKILL.md'), 'name: navigation\n');
  ...
  const exec = async (_command, args) => {
    ...
    return { ok: true, stderr: '', code: 0,
      stdout: `navigation [Enabled]\n  Location:    ${skill}\n` };
  };
  const result = await inspectGeminiIntegration({
    exec, home, expectedPluginVersion: '0.3.0', expectedManagedVersion: '0.9.0',
  });
  assert.equal(result.state, 'active');
...
test('Gemini managed skills are inactive when the host does not discover them', async () => {
  ...
  const result = await inspectGeminiIntegration({
    exec, home, expectedManagedVersion: '0.9.0',
  });
